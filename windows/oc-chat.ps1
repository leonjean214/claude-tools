...
try {
    ...
    if ($ollamaModel -eq "gemma4") {
        $sid = [System.Guid]::NewGuid().ToString()
        $argList = @("agent", "--agent", "main", "--session-id", $sid, "--timeout", $Timeout, "-m", $lastUserMsg)
        openclaw @argList
        exit $LASTEXITCODE
    }
    exit 1
